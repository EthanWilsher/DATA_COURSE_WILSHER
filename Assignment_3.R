...
irisHW <- read_csv("Desktop/DATA_COURSE_WILSHER/Data/iris.csv")

irisHW[seq(2,150,2),]

# 2.  Create a new object called iris_chr which is a copy of iris, except where every column is a character class

write.csv(df,"Desktop/DATA_COURSE_WILSHER/Data/iris_copy.csv")
iris_chr <- read.csv("Desktop/DATA_COURSE_WILSHER/Data/iris_copy.csv")
...
names(irisHW)
Sepal.Area <- irisHW$Sepal.Length * irisHW$Sepal.Width
# 4.  Add Sepal.Area to the iris data frame as a new column

irisHW["Sepal.Area"] <- Sepal.Area

# 5.  Create a new dataframe that is a subset of iris using only rows where Sepal.Area is greater than 20 
      # (name it big_area_iris)

big_area_iris <- irisHW[Sepal.Area > 20,]
